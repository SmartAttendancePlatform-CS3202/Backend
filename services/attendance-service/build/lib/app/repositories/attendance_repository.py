...
def create_session(db: Session, data: dict) -> LectureSession:
    # Auto-increment session number for this offering
...
def get_sessions(db: Session, offering_id: UUID = None, skip: int = 0, limit: int = 100) -> List[LectureSession]:
...
def close_session(db: Session, session_id: UUID) -> Optional[LectureSession]:
    ...
    if session:
        ...
        session.held_at = session.scheduled_at # or now? 
        db.commit()
        db.refresh(session)
    return session
...
def get_open_window(db: Session, lecture_session_id: UUID, window_type: str) -> Optional[VerificationWindow]:
    ...
    return db.query(VerificationWindow).filter(
        ...
        VerificationWindow.window_type == window_type,
        VerificationWindow.is_active == True,
        VerificationWindow.actual_opened_at <= now,
        (VerificationWindow.actual_closed_at == None) | (VerificationWindow.actual_closed_at > now)
    ).first()
...
def upsert_first_check_in(db: Session, lecture_session_id: UUID, student_id: UUID, attempt_data: dict) -> AttendanceRecord:
    ...
    if not record:
        record = AttendanceRecord(
            ...
            first_check_in_at=datetime.utcnow()
        )
        db.add(record)
    else:
        record.first_check_in_at = datetime.utcnow()
...
def update_random_check_status(db: Session, verification_window_id: UUID, student_id: UUID, attempt_data: dict) -> AttendanceRecord:
...
def get_attendance_records(db: Session, session_id: UUID = None, student_id: UUID = None) -> List[AttendanceRecord]:
...
def update_attendance_record(db: Session, record_id: UUID, update_data: dict) -> AttendanceRecord:
