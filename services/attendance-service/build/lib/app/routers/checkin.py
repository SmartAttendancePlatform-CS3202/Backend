...
CHECKIN_ATTEMPTS = Counter(
    ...
    "Total number of student attempts categorized by status",
    ["status"]
)

RANDOM_CHECK_ATTEMPTS = Counter(
    "random_check_attempt_count",
    "Total number of random check attempts categorized by reason",
    ["reason"]
)
...
def record_check_in_tick(
    ...
    CHECKIN_ATTEMPTS.labels(status="attempts").inc()
    try:
        result = attendance_service.record_check_in(db, current_user.id, payload)
        CHECKIN_ATTEMPTS.labels(status="success").inc()
        return result
    except HTTPException as e:
        CHECKIN_ATTEMPTS.labels(status="failed").inc()
        raise e
